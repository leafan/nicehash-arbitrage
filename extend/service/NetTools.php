<?php
/**
 * @note    modified by leafan for net connection use.
 */
namespace service;
...
class NetTools
{
    ...
    public static function json2arr($json)
    {
        ...
        if (empty($result)) {
            throw new HttpException(404, 'invalid response.');

        }
        if (!empty($result['errcode'])) {
            throw new HttpException(500, "errmsg is: " . $result['errmsg'] . ", error code is: " 
                                . $result['errcode']    . ", full result is: " . $result);
        }
    ...
    protected static function doRequest($method, $url, $options = [])
    {
        ...
        if (!empty($options['ssl_cer'])) {
            if (file_exists($options['ssl_cer'])) {
               ...
               throw new HttpException(500, 'Certificate files that do not exist. --- [ssl_cer]');
            }
        }
        // 证书文件设置
    ...
    public static function pushFile($name, $content)
    {
        ...
        if (!file_put_contents($file, $content))
            throw new HttpException(500, 'local file write error.');
        return $file;
    }
    ...
    public static function setCache($name, $value = '', $expired = 3600)
    {
        ...
        if (!file_put_contents($file, $content)) 
            throw new HttpException(500, 'local cache error.');
        return $file;
    }
